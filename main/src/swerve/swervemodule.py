...
class SwerveModule(ISwerveModule):
    ...
    _drive_motor: rev.SparkMax
    _angle_motor: rev.SparkMax

    drive_motor_encoder: rev.SparkRelativeEncoder
    angle_motor_encoder: rev.SparkRelativeEncoder

    drive_pid: rev.SparkClosedLoopController
    angle_pid: rev.SparkClosedLoopController
    ...
    @property
    def drive_motor(self) -> rev.SparkMax:
        """Don't use this unless for testing"""
        return self._drive_motor

    @property
    def angle_motor(self) -> rev.SparkMax:
        """Don't use this unless for testing"""
    ...
    def angle_pid(self) -> rev.SparkClosedLoopController:
        """This is exposed only for SmartDashboard to edit PID values"""
        return self.angle_pid

    def drive_pid(self) -> rev.SparkClosedLoopController:
        """This is exposed only for SmartDashboard to edit PID values"""
    ...
    def __init__(self, position: ModulePosition, module_config: SwerveModuleConfig, physical_config: PhysicalConfig,
        ...
        self._drive_motor = rev.SparkMax(module_config.drive_motor.id, rev.SparkMax.MotorType.kBrushless)
        self._angle_motor = rev.SparkMax(module_config.angle_motor.id, rev.SparkMax.MotorType.kBrushless)
    ...
    def velocity(self, meters_per_sec: float):
        ...
        self.drive_pid.setReference(meters_per_sec, rev.SparkMax.ControlType.kVelocity)
    ...
    def angle(self, angle: float):
        ...
        if self.rel_to_corrected_angle_adjustment is None or self.rel_to_corrected_angle_adjustment == 0:
            self.angle_pid.setReference(math_help.wrap_angle(angle), rev.SparkMax.ControlType.kPosition)
    ...
    def rotate_drive_wheel(self, num_rotations: float):
        ...
        self.drive_pid.setReference(pos + num_rotations, rev.SparkMax.ControlType.kPosition)
    ...
    def drive_set_distance(self, meters: float, angle: float):
        ...
        self.drive_pid.setReference(meters, rev.SparkMax.ControlType.kPosition)
        self.angle = angle
